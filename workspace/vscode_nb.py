...
def dataGen(data):
    if 'FLAG_BusinessHour' not in data.columns:
        bt = businesstime.BusinessTime(business_hours = (datetime.time(9), datetime.time(18)),                                       weekends=(5, 6), holidays=USFederalHolidays())
        dates, isBD = [],[]
...
def entitySet(data):
    ...
    es = es.entity_from_dataframe(index='index', entity_id='log',                                   dataframe = data,                                   variable_types={f: ft.variable_types.Categorical 
